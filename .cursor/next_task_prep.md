...
- **task**: begin `TASK-0023B` **Phase 5** under a new Cursor-internal plan `task-0023b_phase-5_<hash>.plan.md` (to be authored at the start of the Phase-5 session as a separate plan file).
- **focus (immediate)**: Cut **P5-01** — `nexus-evidence` crate skeleton + canonicalization spec + unit tests for deterministic hashing.
- **mode**: switch to plan mode first (author the Phase-5 plan), then agent mode to execute the 6 cuts (P5-01 → P5-06) sequentially with the Phase-4 Proof-Floor (which now requires `verify-uart` clean, `arch-gate` 6/6 rules clean, and `PROFILE=full` byte-identical marker ladder) after each.
- **closed in current session**: all 10 Phase-4 cuts (P4-01 → P4-10). RFC-0038 §"Stop conditions / acceptance" Phase-4 checklist ticked (10 boxes). `proof-manifest.toml` is the SSOT for marker ladder (433 entries) + harness profiles (`full / smp / dhcp / dhcp-strict / os2vm / quic-required`) + runtime profiles (`bringup / quick / ota / net / none`). New host-only crate `nexus-proof-manifest` (parser + CLI: `list-markers / list-env / list-forbidden / list-phases / verify / verify-uart`); `selftest-client/build.rs` generates `markers_generated.rs`; 373 emit sites across 29 files now reference `crate::markers::M_<KEY>` constants. `arch-gate` is 6/6 mechanical rules — Rule 6 (added in P4-10) forbids `REQUIRE_*` env literals in `test-*`/`ci-*` justfile recipes. `scripts/qemu-test.sh` consumes the manifest (env wiring + mirror-check + verify-uart post-pass); `tools/os2vm.sh` consumes the manifest (subset mirror-check). `os_lite/profile.rs` + `run_or_skip!` macro implement runtime phase skipping with `dbg: phase X skipped` breadcrumbs. `just test-os PROFILE=…` is canonical; `test-smp`/`test-os-dhcp`/`test-os-dhcp-strict`/`test-dsoftbus-2vm`/`test-network` deleted in P4-10. `TASK-0024` unblocked. QEMU `SELFTEST:` ladder for `PROFILE=full` byte-identical to pre-Phase-4 baseline.
...
## Ready condition
- **Active plan (Cursor-internal, do not edit during execution)**: TBD — `/home/jenning/.cursor/plans/task-0023b_phase-5_<hash>.plan.md` to be authored at the start of the Phase-5 session as a separate plan file, scoped to 6 cuts only (P5-01 → P5-06), signed-evidence-bundle-driven, mirroring the Phase-3 / Phase-4 plan format.
- **Resume command (when user says "go")**: switch to **plan mode** to author the Phase-5 plan; then switch to agent mode, mark P5-01 todo `in_progress`, and execute the `nexus-evidence` crate skeleton + canonicalization spec + deterministic-hash unit tests. After each cut: `RUSTFLAGS='--cfg nexus_env="os" -W unexpected_cfgs -W dead_code' cargo check -p selftest-client --no-default-features --features os-lite --target riscv64gc-unknown-none-elf` → `cargo test -p dsoftbusd -- --nocapture` → `just test-dsoftbus-quic` → `just test-os PROFILE=full` (verify-uart wired; ladder must be byte-identical vs pre-cut baseline) → `cargo test -p nexus-proof-manifest -- --nocapture` → `cargo test -p nexus-evidence -- --nocapture` (from P5-01 onward) → `rustfmt +stable <touched .rs>` → `just dep-gate` (chains `arch-gate` first; arch-gate is now 6/6 rules) → `just lint`. From P5-04 onward also `tools/verify-evidence.sh target/evidence/<latest>` returns 0.
- **Phase 5 closure trigger**: tick RFC-0038 Phase-5 checklist (6 boxes); sync `.cursor/{handoff/current.md, next_task_prep.md, current_state.md}`; open `task-0023b_phase-6_<hash>.plan.md` (6 cuts, replay capability); refresh `tasks/STATUS-BOARD.md`, `tasks/IMPLEMENTATION-ORDER.md`, `docs/testing/index.md`.
