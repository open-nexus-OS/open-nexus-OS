...
- **task**: `TASK-0023B` **Phase 6** is functionally closed (2026-04-20). The next executable step is environmental, not behavioural: run the sealed-bundle replay on the external project CI runner per `docs/testing/replay-and-bisect.md` §7-§8 and archive `.cursor/replay-ci.{json,log}`. Once that lands, flip P6-05 status + tick RFC-0038 Phase-6 checkbox + mirror across STATUS-BOARD / IMPLEMENTATION-ORDER / `.cursor/{handoff/current,current_state}.md`. After that, queue head moves to `TASK-0024`.
- **mode**: no further code, fixture, or doc changes are pending for Phase 6. The closure step is purely "execute the replay on the CI runner, archive the two artifacts, and apply the documented status flip".
- **closed in current session (2026-04-20)**:
  - Phase-6 tooling cuts P6-01/02/03/04/06 delivered (replay/diff/bisect/CI-wrapper/docs).
  - P6-05 mechanically done: native + containerized CI-like exact-match replays + synthetic bad-bundle classified diff + 3-commit good→drift→regress bisect smoke (extended `--synthetic-map` with `good|drift|bad` verdict).
  - All Phase-6 hard gates verified locally (`--max-seconds`/`--max-commits` mandatory, env override rejected).
  - All status documents (task / RFC / STATUS-BOARD / IMPLEMENTATION-ORDER / `.cursor/{handoff/current,current_state,next_task_prep}`) synced to "Phase 6 functionally closed; only external CI artifact remaining".
...
## Ready condition
- **Active plan (Cursor-internal, do not edit during execution)**: `/home/jenning/.cursor/plans/task-0023b-phase6-cut-loop_1d8498c9.plan.md` (single-cut cadence preserved).
- **Resume command (when user says "go")**: run the missing external CI replay proof for P6-05:
  1. CI host: `tools/replay-evidence.sh target/evidence/20260420T133203Z-full-b84e4c2.tar.gz --max-seconds=300 --log-file=.cursor/replay-ci.log --report-json=.cursor/replay-ci.json`
  2. Compare `.cursor/replay-ci.json` with existing `.cursor/replay-dev-a.json` + `.cursor/replay-ci-like.json` (all must be exact-match modulo allowlist).
- **Phase 6 closure trigger**: after external CI replay evidence capture, tick RFC-0038 Phase-6 checklist; sync `.cursor/{handoff/current.md, next_task_prep.md, current_state.md}`; refresh `tasks/STATUS-BOARD.md`, `tasks/IMPLEMENTATION-ORDER.md`; mark `TASK-0023B` as **CLOSED**.

### Open closure gates (execution checklist)

- [ ] CI runner replay command exits `0`.
- [ ] `.cursor/replay-ci.json` shows `trace_diff.status = exact_match`.
- [ ] `.cursor/replay-ci.json` shows `config_compare.ok = true`.
- [ ] CI + dev replay artifacts are archived together for review.
- [ ] Task/RFC/status docs flipped from "P6-05 pending" to "Phase 6 closed".
