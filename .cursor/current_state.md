...
- **last_decision**: `TASK-0023B` Phase 3 **CLOSED** under Cursor-internal plan `task_0023b_phase_3_ee96d119.plan.md`. All 4 cuts (P3-01 → P3-04) executed; RFC-0038 Phase-3 checklist ticked (4 boxes). `main.rs` shrunk 122 → **49 LoC** (dispatch-only); `host_lite.rs` (78 LoC) holds host-pfad `run()` symmetric to `os_lite::run()`; 13 single-file `name/mod.rs` modules flattened to `name.rs` via pure `git mv` (no parent edits, history preserved); `scripts/check-selftest-arch.sh` (167 LoC, 5 mechanical rules) + `just arch-gate` recipe chained into `just dep-gate`; `source/apps/selftest-client/.arch-allowlist.txt` (3 sections: marker_emission/mod_rs_fn/size_500) baselines current escapes for Phase 4 tightening. Standards review: `#[must_use]` redundant on `Result` fns (`core::result::Result` already `#[must_use]`); `Slot(u32)` newtype deferred to Phase 4 with explicit `TODO(TASK-0023B Phase 4)` note in `os_lite/context.rs` (~16 call sites across 8 files; non-mechanical); Send/Sync intent comment added to `os_lite/context.rs` documenting single-HART/single-task runtime invariant (no marker traits introduced — adding them later without changing the runtime model would mask, not reveal, a real concurrency bug). QEMU `SELFTEST:` ladder byte-identical (119 markers) across all 4 cuts versus pre-Phase-3 baseline. Phase 4 = 10 cuts (manifest-driven); `TASK-0024` still blocked on Phase 4 closure.
...
- **active_task**: `TASK-0023B` Phase 4 (10 cuts: P4-01 → P4-10). Phase 3 closed 2026-04-17.
- **active_plan**: TBD — author `task-0023b_phase-4_<hash>.plan.md` (Cursor-internal) at the start of the Phase-4 session; scope: 10 cuts only (manifest-driven).
- **resume cut**: P4-01 — write manifest schema doc (`docs/testing/proof-manifest.md`) + `proof-manifest.toml` skeleton (meta + 12 phase declarations) + new host-only crate `nexus-proof-manifest` with parser + reject tests.
- **seed_contract**:
...
## Next handoff target
- **Resume command (when user says "go")**: switch to plan mode and author `task-0023b_phase-4_<hash>.plan.md` (Cursor-internal). Scope: 10 cuts (P4-01 → P4-10), manifest-driven. Mirror Phase-2/Phase-3 plan format. Then switch to agent mode and execute P4-01 first.
- **Cut order (Phase 4, locked in RFC-0038)**: P4-01 (manifest schema doc + `proof-manifest.toml` skeleton + new host-only crate `nexus-proof-manifest` parser + reject tests) → P4-02 (RFC-0014 binding doc; manifest is normative SSOT for marker ladder) → P4-03 (populate manifest with all 119 markers + `build.rs` generates `markers_generated.rs`) → P4-04 (replace marker emission in `phases/*` with generated constants; tighten `arch-gate` rule 3 to "only `markers_generated.rs` + `markers.rs`"; shrink `[marker_emission]` allowlist to zero) → P4-05 (`scripts/qemu-test.sh` consumes manifest via `nexus-proof-manifest list-markers --profile=…`) → P4-06 (migrate `just test-*` recipes to `PROFILE=…`) → P4-07 (`tools/os2vm.sh` consumes manifest) → P4-08 (runtime profiles via `SELFTEST_PROFILE` env / kernel cmdline + new `os_lite/profile.rs`) → P4-09 (deny-by-default analyzer in `nexus-proof-manifest`) → P4-10 (deprecate direct `REQUIRE_*` env-var usage in CI; closure).
- **Per-cut verification floor (Phase 4)**: `RUSTFLAGS='--cfg nexus_env="os" -W unexpected_cfgs -W dead_code' cargo check -p selftest-client --no-default-features --features os-lite --target riscv64gc-unknown-none-elf` → `cargo test -p dsoftbusd -- --nocapture` → `just test-dsoftbus-quic` → `REQUIRE_DSOFTBUS=1 RUN_UNTIL_MARKER=1 RUN_TIMEOUT=220s just test-os` (or `just test-os PROFILE=…` once P4-06 lands; compare `grep -E '^SELFTEST: '` against pre-cut baseline; must be byte-identical) → `cargo test -p nexus-proof-manifest -- --nocapture` (from P4-01) → `rustfmt +stable <touched .rs>` → `just dep-gate` (chains `arch-gate` first) → `just lint`.
- **Phase 4 closure**: tick RFC-0038 Phase-4 checklist (10 boxes); sync `.cursor/{handoff/current.md, next_task_prep.md, current_state.md}`; open `task-0023b_phase-5_<hash>.plan.md` (6 cuts, signed evidence); unblock `TASK-0024` (update its `depends-on`); refresh `tasks/STATUS-BOARD.md`, `tasks/IMPLEMENTATION-ORDER.md`, `docs/testing/index.md`.
