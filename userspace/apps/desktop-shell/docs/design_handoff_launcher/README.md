...
## What's in here
- **`LAUNCHER-SPEC.html`** — **die Detail-Spec: öffne diese Datei zuerst.** Jede Zahl für Startmenü (klein/groß), Taskbar, Docks, Icon-Arbeitsfläche, Aktiv-Zustände, Hover/Press, Label-Schatten und Animationen — mit live gerendertem Glas-Beispiel neben jeder Regel. Lädt die Tokens aus `reference/tokens/` direkt, ist also selbst der Beweis, dass die Tokens stimmen.
- **`assets/icons/`** — alle im Launcher verwendeten App- und Ordner-Icons als SVG, plus `README.md` mit der Varianten-Zuordnung (native / wrapped / freestanding).
...
## Kurzreferenz der wichtigsten Zahlen
(Vollständig mit Beispielen in `LAUNCHER-SPEC.html`.)

| Ding | Wert |
|---|---|
| Topbar / Taskbar Höhe | 36px / 56px |
| Icon-Größen (Tile/Radius/Label) | xs 40/11/10 · sm 48/13/11 · md 56/16/13 · lg 72/20/14 · xl 88/24/15 |
| Arbeitsfläche Desktop | flex-column + wrap, `gap 22 × 34`, Padding 20, md-Icons |
| Arbeitsfläche Tablet | `repeat(6,1fr)`, `gap 24 × 16`, Padding 28, md-Icons |
| Aktive App Taskbar | Unterstrich 20 × 2px, `--glass-text-primary` @ 0.7 |
| Aktive App Dock | Punkt 4 × 4px, gleiche Farbe |
| Startmenü Desktop klein | 720 × 520, Radius 24, Padding 24, Grid `repeat(4,1fr)` `gap 12 × 8`, **kein** Backdrop-Blur, Origin `center bottom` |
| Startmenü Fullscreen | 100%, Scrim + `blur(24px)`, xl-Icons, gepagert |
| Pager-Zelle | `CELL_W = tile+56`, `CELL_H = tile+66`; cols/rows = `floor(container / CELL)` |
| Wisch-Schwelle | 16% der Containerbreite · Drag ab 4px · Rad ab 30, 360ms Sperre |
| Icon-Hover / Press | `scale(1.08)` / `scale(0.92)`, `cubic-bezier(0.34,1.56,0.64,1)` 0.2s |
| Label-Halo | dark: dunkler Halo · light: heller Halo (`--glass-label-shadow`) — nur auf Wallpaper, nie in Glas |
