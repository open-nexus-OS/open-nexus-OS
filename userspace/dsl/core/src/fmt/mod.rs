...
use crate::ast::{
    AssignOp, Decl, File, HandlerAction, HandlerDecl, ModifierCall, Stmt, ViewNode, WidgetNode,
};
use alloc::string::String;

mod expr;
use expr::{call_args_to, expr_to, pattern_to, type_to};
...
pub(super) fn push_escaped(out: &mut String, s: &str) {
    for ch in s.chars() {
        match ch {
            '"' => out.push_str("\\\""),
            '\\' => out.push_str("\\\\"),
...
fn widget_to(out: &mut String, widget: &WidgetNode, level: usize) {
    ...
    // The callsite slot block is its own brace block after the prop block —
    // an unbound `Comp { } { content { … } }` keeps both, so re-parsing sees
    // the same shape.
    ...
    if has_block || !widget.slot_bodies.is_empty() {
