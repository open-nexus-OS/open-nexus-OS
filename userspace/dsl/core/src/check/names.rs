...
use super::modifiers::check_modifiers;
use super::queries::{check_query_call, check_query_decl};
use super::Model;
use crate::ast::{Decl, Expr, File, HandlerAction, Pattern, Stmt, ViewNode};
...
pub(super) fn check_view(node: &ViewNode, model: &Model<'_>, diags: &mut Vec<Diagnostic>) {
    match node {
        ViewNode::Widget(widget) => {
            ...
            // Slot bodies are ordinary views — the slot-specific rules
            // (binding a declared slot, caller scope, …) live in `slots.rs`.
        ...
        // Resolved against the enclosing component's declarations in `slots.rs`.
        ViewNode::Slot { .. } => {}
    }
}
