...
fn stream_lifecycle_open_ack_close_rst_is_deterministic() {
    ...
    let opened = session
        .apply_inbound_frame(sid, priority(1), InboundFrame::Open)
        .expect("open frame");
    assert_eq!(
        opened,
        FrameApplyOutcome::StreamOpened {
    ...
    assert_eq!(
        acked,
        FrameApplyOutcome::OpenAcked {
            state: StreamState::Open
        }
    );

    let closed_remote = session
        .apply_inbound_frame(sid, priority(1), InboundFrame::Close)
        .expect("close frame");
    assert_eq!(
        closed_remote,
        FrameApplyOutcome::StreamTransitioned {
            state: StreamState::HalfClosedRemote
        }
    );

    let reset = session
        .apply_inbound_frame(sid, priority(1), InboundFrame::Rst)
        .expect("rst frame");
    assert_eq!(
        reset,
        FrameApplyOutcome::StreamTransitioned {
...
fn data_frame_rejects_oversize_payload() {
    ...
    let sid = stream_id(9);
    let _ = session
        .apply_inbound_frame(sid, priority(0), InboundFrame::Open)
        .expect("open frame");
...
fn keepalive_pong_resets_timeout_budget() {
    ...
    let sid = stream_id(1);
    session
        .open_stream(sid, priority(0), WindowCredit::new(32))
        .expect("open stream");

    assert_eq!(session.keepalive_tick(3), KeepaliveVerdict::SendPing);
    let pong = session
        .apply_inbound_frame(sid, priority(0), InboundFrame::Pong)
        .expect("pong frame");
...
fn window_update_frame_applies_credit_delta() {
    ...
    assert_eq!(
        updated,
        FrameApplyOutcome::WindowUpdated {
            credit: WindowCredit::new(65_568)
        }
    );
}
...
fn seeded_state_machine_sequence_preserves_credit_invariants() {
    ...
    session
        .open_stream(sid, priority(2), WindowCredit::new(64))
        .expect("open stream");

    let first_send = session.send_data(sid, 24).expect("first send");
    assert_eq!(
        first_send,
        SendBudgetOutcome::Sent {
            remaining_credit: WindowCredit::new(40)
        }
    );
    ...
    assert_eq!(
        second_send,
        SendBudgetOutcome::Sent {
            remaining_credit: WindowCredit::new(0)
        }
    );
...
fn rst_transition_is_idempotent_and_close_after_rst_rejects() {
    ...
    session
        .open_stream(sid, priority(1), WindowCredit::new(32))
        .expect("open stream");

    let first_rst = session
