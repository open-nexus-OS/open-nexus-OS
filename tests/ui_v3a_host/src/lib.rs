...
mod tests {
    ...
    use nexus_layout_types::{
        ...
        ShapeKind, TextAlign, TextContent, TextNode, TextStyle, VisualStyle, WhiteSpace,
    };
    ...
    fn layout_to_golden(node: &LayoutNode, width: i32) -> Vec<GoldenBox> {
        let r = LayoutEngine::new()
            .layout(node, px(width), &MockMeasure { char_width: px(0) })
            .unwrap();
        r.boxes
            .iter()
            .map(|b| GoldenBox {
    ...
    #[test]
    fn proof_panel_layout_is_independent_of_cursor_motion() {
        let mut moved = default_visible_state();
        moved.cursor_x = 720;
        moved.cursor_y = 512;
        ...
        assert_eq!(
            ...
            "cursor-only input must not invalidate or reshape the proof panel layout"
        );
    }
    ...
    fn proof_panel_geometry_is_independent_of_transient_target_state() {
        ...
        let inactive_geometry: Vec<GoldenBox> = inactive
            ...
            .collect();
        let active_geometry: Vec<GoldenBox> = active
            .boxes
            .iter()
            .map(|b| GoldenBox {
                node_id: b.node_id,
        ...
        assert_eq!(
            inactive_geometry, active_geometry,
            "hover/click/wheel/key state may repaint the panel but must not require a layout rebuild"
        );
    }
