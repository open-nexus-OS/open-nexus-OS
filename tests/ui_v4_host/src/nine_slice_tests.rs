...
mod tests {
    use nexus_effects::{
        composite_drop_shadow, composite_nine_slice_shadow, EffectBudget, EffectCache,
        NineSliceShadow,
    };
    ...
    fn solid_alpha_mask(w: u32, h: u32) -> Vec<u8> {
    ...
    fn test_nine_slice_produces_output() {
        ...
        let count = composite_nine_slice_shadow(
            &mut target, 200, 200, stride,
            elem_w, elem_h, &shadow,
            50, 60, &mut budget, None,
        );
    ...
    fn test_nine_slice_corners_blurred() {
        ...
        let sharp = NineSliceShadow { corner_size: 10, blur_radius: 0, spread: 0, color: black_shadow() };
        let blurred = NineSliceShadow { corner_size: 10, blur_radius: 4, spread: 0, color: black_shadow() };
        let mut budget = EffectBudget::default();
        let _ = composite_nine_slice_shadow(
            &mut target, 120, 120, stride,
            elem_w, elem_h, &blurred,
            30, 30, &mut budget, None,
        );
    ...
    fn test_nine_slice_different_params_different_cache_key() {
        let (mut target1, stride) = make_target(120, 120);
        let (mut target2, _) = make_target(120, 120);
        let shadow1 = NineSliceShadow { corner_size: 8, blur_radius: 2, spread: 0, color: black_shadow() };
        let shadow2 = NineSliceShadow { corner_size: 10, blur_radius: 4, spread: 0, color: black_shadow() };
        let mut budget = EffectBudget::default();
        let mut cache = EffectCache::with_capacity(16);

        let _ = composite_nine_slice_shadow(
            &mut target1, 120, 120, stride, 50, 50, &shadow1, 35, 35, &mut budget, Some(&mut cache),
        );
        let _ = composite_nine_slice_shadow(
            &mut target2, 120, 120, stride, 50, 50, &shadow2, 35, 35, &mut budget, Some(&mut cache),
        );
    ...
    fn test_nine_slice_vs_full_blur_similar() {
        ...
        let _ = composite_nine_slice_shadow(
            &mut target_nine, 160, 160, stride,
            elem_w, elem_h, &shadow,
            40, 50, &mut budget, None,
        );
        ...
        let _ = composite_drop_shadow(
            &mut target_full, 160, 160, stride,
            &alpha, 0, 0, black_shadow(), &mut budget2,
        );
        ...
        for i in (3..target_nine.len()).step_by(4) {
            if target_nine[i] > 0 { nine_pixels += 1; }
            if target_full[i] > 0 { full_pixels += 1; }
        }
        ...
        assert!(ratio > 0.3, "9-slice should cover reasonable portion of full blur area (ratio={ratio:.2})");
    }
}