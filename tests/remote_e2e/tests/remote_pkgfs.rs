...
fn remote_pkgfs_roundtrip_and_negative_statuses() {
    ...
    let bytes = connection
        ...
        .expect("pkgfs read once");
    assert!(bytes.starts_with(b"ro.nexus.build=dev"));

    let (st_missing, _, _) = connection
        .remote_pkgfs_stat_status("pkg:/system/does-not-exist")
        .expect("stat missing");
    assert_eq!(st_missing, 5, "expected PK_STATUS_NOT_FOUND");

    let (st_traversal, _, _) = connection
        .remote_pkgfs_stat_status("pkg:/system/../secret")
        .expect("stat traversal");
    assert_eq!(st_traversal, 3, "expected PK_STATUS_PATH_TRAVERSAL");

    let (st_scheme, _, _) = connection
        .remote_pkgfs_stat_status("file:/etc/passwd")
        .expect("stat non-packagefs");
    assert_eq!(st_scheme, 4, "expected PK_STATUS_NON_PACKAGEFS_SCHEME");

    let long_rel = "a".repeat(193);
    let long_path = format!("pkg:/{}", long_rel);
    let (st_oversized, _, _) = connection
        .remote_pkgfs_stat_status(&long_path)
        .expect("stat oversized");
    assert_eq!(st_oversized, 7, "expected PK_STATUS_OVERSIZED");

    let (st_open, handle) = connection
        .remote_pkgfs_open_status("pkg:/system/build.prop")
        .expect("open build.prop");
    ...
    let st_badf = connection
        .remote_pkgfs_close_status(handle)
        .expect("close badf");
    ...
    for h in open_handles {
        let st = connection
            .remote_pkgfs_close_status(h)
            .expect("close opened handle");
