...
fn normal_usernames() -> Vec<String> {
    // Platzhalter: später wieder echte Userliste einbauen
    vec!["nexus".to_string()]
}
...
fn login_command(
    ...
    // Platzhalter: keine Verifikation, starte direkt die Session
    let mut cmd = Command::new(launcher_cmd);
    cmd.args(launcher_args.iter());
    Some(cmd)
}
...
fn login_window(launcher_cmd: &str, launcher_args: &[String]) -> Result<Option<Command>, String> {
    ...
    let image = Image::from_path("/ui/login.png")
        .or_else(|_| Image::from_path("/ui/nexus-assets/login.png"))?;
...
fn main() -> io::Result<()> {
    ...
    let _ = RedoxLogger::new()
        ...
        .with_process_name("nexus_login".into())
        .enable();
