...
// Process spawning and small helpers around wait/reap.

use std::process::{Child, Command};
use std::io::{ErrorKind, Result as IoResult};

/// Convert Exec string to a Command with proper argument parsing.
/// Supports %f/%F/%u/%U (path/url) for simple cases.
...
pub fn spawn_exec(exec: &str, path_opt: Option<&str>) {
    match exec_to_command(exec, path_opt) {
        ...
        None => log::error!("failed to parse {}", exec),
    }
}
...
/// Non-blocking wait helper (platform-specific).
...
pub fn reap_all_zombies() {
    log::debug!("Reaping all zombie processes");
    let mut status = 0;
    while wait(&mut status).is_ok() {}
}

/// Kill and wait all children.
pub fn cleanup_children(children: &mut Vec<(String, Child)>) {
    log::info!("Cleaning up {} child processes", children.len());
    for (exec, child) in children.iter_mut() {
        let pid = child.id();
        match child.kill() {
            Ok(()) => log::info!("Successfully killed child: {}", pid),
            Err(err) => log::error!("failed to kill {} ({}): {}", exec, pid, err),
        }
        match child.wait() {
            Ok(status) => log::info!("{} ({}) exited with {}", exec, pid, status),
            Err(err) => log::error!("failed to wait for {} ({}): {}", exec, pid, err),
        }
    }
    children.clear();
}

/// Reap finished children in-place.
pub fn reap_children(children: &mut Vec<(String, Child)>) {
    ...
    while i < children.len() {
        let remove = match children[i].1.try_wait() {
            Ok(None) => false,
            Ok(Some(status)) => {
                log::info!("{} ({}) exited with {}", children[i].0, children[i].1.id(), status);
                true
            }
            Err(err) => {
                log::error!("failed to wait for {} ({}): {}", children[i].0, children[i].1.id(), err);
                true
            }
        };
