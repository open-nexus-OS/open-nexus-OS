...
// Switched to services::app_catalog as unified package source.
...
use crate::services::app_catalog::{get_packages, filter_packages_for_path};
...
pub fn chooser_main(paths: env::Args) {
    for ref path in paths.skip(1) {
        ...
        if packages.len() > 1 {
            ...
            .expect("launcher: failed to open chooser window");
            ...
            'choosing: loop {
                for event in window.events() {
                    ...
                    if redraw {
                        ...
                        let mut y = 0;
                        for (i, _p) in packages.iter().enumerate() {
