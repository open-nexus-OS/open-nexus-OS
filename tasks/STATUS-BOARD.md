...
Current queue head: `TASK-0065` / UI v6b app lifecycle + notifications + navigation — **P0–P3 + P4a done** (RFC-0065 + ADR-0036/0037; `bundlemgrd enumerate`; `abilitymgr` real service + lifecycle broker; **real `.nxb` app bundles**; `handoff.rs` launch orchestrator; **per-app-surface model** — `windowd::destroy_surface` + host-tested `app_surface::AppSurfaces`, own VMO per app, lazy load/free, own layer). 21 abilitymgr + 3 nxb-pack + 115 windowd tests, riscv-checked. **P4b-1 done**: `userspace/apps/search` (`search-app`) is a real app crate — owns its word list + filter + renders its own surface buffer; 10 host tests. **Boot-gated next (P4b-2)**: compositor composites the search client surface (remove baked `self.search`) + live `SurfaceBinder`→windowd IPC, then chat. Recommend a boot+commit of P0–P4b-1 before the compositor swap.
Current contract status: `RFC-0058` is `Done`; `RFC-0059` is `Complete`; `RFC-0063` is `Complete`; `RFC-0064` is `Complete`; `RFC-0065` is `Draft` (seed; SSOT = TASK-0065); `TASK-0064` WM v1 proven (Crossfade/Slide deferred to TASK-0064B).
