...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bundle_version = args
        .next()
        .ok_or_else(|| usage("missing <bundle_version>"))?;
    let publisher_hex = args
        .next()
        .ok_or_else(|| usage("missing <publisher_hex>"))?;
    let payload_sha256 = args
        .next()
        .ok_or_else(|| usage("missing <payload_sha256>"))?;
    let payload_size = args
        .next()
        .ok_or_else(|| usage("missing <payload_size>"))?
        .parse::<u64>()
        .map_err(|_| usage("invalid <payload_size>"))?;
    let manifest_sha256 = args
        .next()
        .ok_or_else(|| usage("missing <manifest_sha256>"))?;
