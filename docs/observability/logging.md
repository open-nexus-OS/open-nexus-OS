...
- If a bounded dump path is available: `execd: minidump written <path>`
- An appended record into `logd` (scope `execd`, bounded message) so selftests can verify crash reporting **without scraping UART**.
  - stable event keys: `event=crash.v1`, `pid`, `code`, `name`, `recent_count`, `recent_window_nsec`
  - crashdump v1 extension keys (when available): `build_id`, `dump_path`
  - marker honesty: minidump success proof requires persisted artifact read-back + decode, not just path presence.
...
- `execd: crash report pid=... code=42 name=demo.minidump`
- `execd: minidump written /state/crash/...`
