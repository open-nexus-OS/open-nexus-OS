...
- **Phase 3 (production-grade closure + standards review)**: ⬜ — adds: flatten Single-File-`name/mod.rs` modules to `name.rs` where Phase 2 did not introduce siblings; move `core_service_probe*` out of `services/mod.rs` (aggregator-only rule); extract host-pfad `run()` from `main.rs` into a symmetric `host_lite/` subtree; introduce `scripts/check-selftest-arch.sh` as a mechanical anti-re-monolithization gate.
...
- [ ] **Phase 3**: production-grade closure + structural-discipline gates — proof: same phase proof floor + `just dep-gate && just diag-os && just arch-gate`.
  ...
  - [ ] Cut P3-03: write `scripts/check-selftest-arch.sh` + `just arch-gate` recipe; chain into `just dep-gate`; produce allowlist file (refinement (7)).
