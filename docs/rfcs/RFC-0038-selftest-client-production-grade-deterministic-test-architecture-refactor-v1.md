...
- Status: Done (2026-04-20; one environmental closure step remains — external CI-runner replay artifact for P6-05, see `docs/testing/replay-and-bisect.md` §7-§11; no code, fixture, or doc work outstanding)
- Owners: @runtime
- Created: 2026-04-16
- Last Updated: 2026-04-20 (Phase 6 functionally closed; proof-floor synthetic bad-bundle + 3-commit good→drift→regress smokes recorded; only environmental external-CI replay artifact remaining)
- Links:
...
- **Phase 4 (Marker-Manifest as Single Source of Truth + profile-aware harness)**: ✅ — closed 2026-04-17. `proof-manifest` is the marker/profile/run-config SSOT and is consumed by `scripts/qemu-test.sh` + `tools/os2vm.sh`; runtime profile switch `SELFTEST_PROFILE=<name>` is live; two-truth marker surface removed.
- **Phase 5 (Signed evidence bundle per QEMU run)**: ✅ — closed 2026-04-17. Every `just test-os PROFILE=…` run writes `target/evidence/<utc>-<profile>-<git-sha>.tar.gz` containing manifest tar + UART + trace.jsonl + config.json + Ed25519 signature.bin. Host-only crate `nexus-evidence` owns canonicalization/sign/verify; `tools/{seal,verify}-evidence.sh` shell wrappers handle key resolution. CI key (env-injected) vs bringup key (file-based, mode 0600) separation enforced via signature label byte; deny-by-default secret scanner refuses to seal bundles with leaked key material. P5-00 prepended to split the proof-manifest into per-phase files before any new code touched it. See "Phase 5 — Signed evidence bundles" below.
- **Phase 6 (Replay capability)**: 🟢 functionally closed (2026-04-20) — `tools/replay-evidence.sh` + `tools/diff-traces.sh` + `tools/bisect-evidence.sh` + `scripts/regression-bisect.sh` shipped with bounded budgets, structured logs, persistent worktree/cargo-cache reuse, and same-SHA `NEXUS_SKIP_BUILD=1` warm-replay (cold ~67s, warm ~14s). Proof floor recorded: empty diff vs good bundle, synthetic bad-bundle classified diff (`.cursor/replay-synthetic-bad.json`), 3-commit good→drift→regress bisect smoke (`.cursor/bisect-good-drift-regress.json`), all hard gates verified (`--max-seconds`/`--max-commits` mandatory, env-override rejected). One environmental item remaining: replay run on the external project CI runner against bundle `target/evidence/20260420T133203Z-full-b84e4c2.tar.gz` per `docs/testing/replay-and-bisect.md` §7-§8; no code, fixture, or doc changes pending.
...
- [x] **Phase 4**: marker manifest as single source of truth + profile-aware harness + runtime selftest profiles. Hard gates per "Phase 4 — Marker-Manifest + profile dimension" above. Closed 2026-04-17 under Cursor-internal plan `task-0023b_phase-4_57f4bce2.plan.md` — 10 cuts landed; QEMU `SELFTEST:` ladder is byte-identical to the pre-Phase-4 baseline under `PROFILE=full`; arch-gate is now 6/6 mechanical rules; `[marker_emission]` allowlist is empty (Rule 3 is allowlist-free); `nexus-proof-manifest` host CLI (`parse + list-markers + list-env + list-forbidden + list-phases + verify + verify-uart`) is the single integration surface for harness scripts and CI.
  - [x] Cut P4-01: wrote manifest schema doc (`docs/testing/proof-manifest.md`) + `proof-manifest.toml` skeleton (meta + 12 `[phase.X]` declarations only) + `source/libs/nexus-proof-manifest/` host-only crate with TOML parser, `Manifest`/`ParseError` types, and 8 reject tests (unknown top-level key, unknown `[meta]` key, missing `schema_version`, missing `default_profile`, duplicate phase, conflicting order).
  - [x] Cut P4-02: ticked RFC-0014 §appendix to declare `proof-manifest.toml` normative for the marker ladder; added 1:1 phase-mapping table to `docs/testing/proof-manifest.md`. No code touched.
  - [x] Cut P4-03: populated `proof-manifest.toml` with 179 gating markers spanning all 12 phases + extended parser with `[marker."…"]` schema, `Marker.const_key()`, profile/phase reference validation. `selftest-client/build.rs` reads the manifest and generates `markers_generated.rs` (`pub(crate) const M_<KEY>: &str = "<literal>";`).
  - [x] Cut P4-04: back-filled an additional 254 diagnostic / fragment / FAIL-label markers (433 total entries) so every literal in the source has a manifest entry, then mass-migrated 373 emit sites across 29 files from `"<literal>"` / `b"<literal>"` to `crate::markers::M_<KEY>` / `M_<KEY>.as_bytes()`. Tightened `arch-gate` Rule 3 to "marker literals only in `markers.rs` + `markers_generated.rs`" (no allowlist); the `[marker_emission]` section in `.arch-allowlist.txt` is now zero entries. Synthetic-violation regression confirmed Rule 3 still fires; reverted clean.
  - [x] Cut P4-05: added `nexus-proof-manifest` host CLI binary (`list-markers / list-env / list-forbidden / list-phases / verify`); populated harness profile bodies (`full / smp / dhcp / os2vm / quic-required`) with `runner` + `extends` + `env`; `scripts/qemu-test.sh` now sources env via `pm_apply_profile_env` and validates its in-script `expected_sequence` against the manifest's `full` projection via `pm_mirror_check`.
  - [x] Cut P4-06: `just test-os PROFILE=…` is the canonical recipe; soft-deprecated `test-smp` / `test-os-dhcp` / `test-dsoftbus-2vm` / `test-network` for one cycle (deleted in P4-10); added `ci-os-full / ci-os-smp / ci-os-dhcp / ci-os-quic / ci-os-os2vm / ci-network` recipes for the CI matrix.
  - [x] Cut P4-07: `tools/os2vm.sh` accepts `--profile=<name>` (default `os2vm`), sources env via `pm_apply_profile_env`, and validates its in-script mux markers via `pm_mirror_subset_check` against the `os2vm` projection. Manifest extended with five `dsoftbus:mux crossvm *` markers (gated `emit_when = { profile = "os2vm" }`) so the mirror check passes.
  - [x] Cut P4-08: added 5 runtime-only profiles (`bringup / quick / ota / net / none`) with explicit `phases = [...]` lists + 12 `dbg: phase X skipped` breadcrumb markers. New `os_lite/profile.rs` (`Profile`, `PhaseId`, `from_kernel_cmdline_or_default`, `includes`, `skip_marker`) drives the `run_or_skip!` macro in `os_lite/mod.rs`. `build.rs` advertises `cargo:rerun-if-env-changed=SELFTEST_PROFILE`. Added `ci-os-runtime-bringup / quick / none` recipes. Manifest tests cover unknown-phase rejection and `runtime_only`-vs-`runner` mutual exclusion.
  - [x] Cut P4-09: added `nexus-proof-manifest verify-uart --profile=<name> --uart=<path>` deny-by-default analyzer (any UART marker that the active profile does not expect, or that it forbids, exits with code 1). Wired into `scripts/qemu-test.sh` post-pass (gated by `PM_VERIFY_UART=1` default). Six new integration tests in `tests/cli_verify_uart.rs` cover clean / unexpected / forbidden / json / missing-file / missing-arg paths.
  - [x] Cut P4-10: deleted the four soft-deprecated aliases (`test-smp / test-os-dhcp / test-os-dhcp-strict / test-dsoftbus-2vm / test-network`); added `ci-os-dhcp-strict` recipe + `[profile.dhcp-strict]` (extends `dhcp` with `REQUIRE_QEMU_DHCP_STRICT=1`); added `arch-gate` Rule 6 ("no `REQUIRE_*` env literal in `test-*` / `ci-*` justfile recipe bodies") with a `[justfile_require_env]` allowlist (currently empty); documented the manifest-driven workflow (profile catalog + add-marker / add-profile / add-phase recipes + verify-uart) in `docs/testing/os-markers.md`. Closure: `.cursor/{handoff,current_state,next_task_prep}` synced to Phase-4-closed / Phase-5-active; `TASK-0024` `depends-on` updated to drop the Phase-4 blocker; `STATUS-BOARD` + `IMPLEMENTATION-ORDER` refreshed; Phase-5 plan to be authored at the start of the Phase-5 session (separate plan, separate session).
- [x] **Phase 5**: signed evidence bundles per QEMU run. Hard gates per "Phase 5 — Signed evidence bundles" above. Closed 2026-04-17 under Cursor-internal plan `task-0023b_phase-5_<hash>.plan.md` — 7 cuts landed (P5-00..P5-06; P5-00 added at session start to split the proof-manifest into per-phase files before any new code touched it). 40 host-side tests in `nexus-evidence` (5 assemble + 6 canonical_hash + 4 key_separation + 5 qemu_seal_gate + 7 scan + 13 sign_verify) lock canonical hashing, signature framing, the 5 tamper classes, key-class separation, the secret-scanner deny-by-default, and the CI seal gate. The seal post-pass is wired into both `scripts/qemu-test.sh` (single bundle) and `tools/os2vm.sh` (per-node A/B bundles) and rejects `NEXUS_EVIDENCE_DISABLE=1` when `CI=1` so a successful run cannot silently drop its audit trail.
  - [x] Cut P5-00: `proof-manifest.toml` (1433 LoC) split into a `source/apps/selftest-client/proof-manifest/` directory tree (`manifest.toml` + `phases.toml` + `markers/*.toml` + `profiles/*.toml`); `[meta] schema_version = "2"` introduced; `nexus-proof-manifest` parser extended with `[include]` glob expansion (lexicographic, conflict-checked) while keeping v1 single-file back-compat. `scripts/qemu-test.sh`, `tools/os2vm.sh`, `selftest-client/build.rs`, and `nexus-proof-manifest` CLI all repointed to `proof-manifest/manifest.toml`. `PROFILE=full` ladder byte-identical to the pre-split baseline.
  - [x] Cut P5-01: new host-only crate `source/libs/nexus-evidence/` with `Bundle` + per-artifact subtypes (`BundleMeta`, `ManifestArtifact`, `UartArtifact`, `TraceEntry`, `ConfigArtifact`), `canonical_hash` (`H(meta) || H(manifest_bytes) || H(uart_normalized) || H(sorted(trace)) || H(sorted(config))`), and 6 integration tests in `tests/canonical_hash.rs` that lock the determinism invariants (sorted trace, sorted env, normalized line endings, `wall_clock_utc` excluded from the hash). Spec authored in `docs/testing/evidence-bundle.md` (artifact schemas, canonicalization rules, error class table).
  - [x] Cut P5-02: `Bundle::assemble` + `nexus_evidence::trace::extract_trace` (substring matching against all manifest literals; `[ts=…ms]` timestamp prefix; deny-by-default for orphan `SELFTEST:` / `dsoftbusd:` lines) + `nexus_evidence::config::gather_config` + reproducible `tar.gz` packing (`mtime=0`, `uid=0`, `gid=0`, mode `0o644`, lex-sorted entries, gzip OS byte fixed) in `bundle_io.rs`. `nexus-evidence` CLI (`assemble / inspect / canonical-hash`) ships behind a hand-rolled arg parser. 5 integration tests in `tests/assemble.rs` cover round-trip, repeated-assembly byte-identity, and trace-extractor behavior (including the deny-by-default reject for unknown `SELFTEST:` markers).
  - [x] Cut P5-03: Ed25519 sign / verify via `ed25519-dalek` (already in the OS graph through `userspace/updates`, so no new forbidden dep landed). Signature wire format frozen at 102 bytes (`magic="NXSE" || version=0x01 || label || hash[32] || sig[64]`); `KeyLabel::{Ci, Bringup}` baked into the signature so `verify --policy=ci` rejects bringup-signed bundles. `Bundle::seal` / `Bundle::verify` added; `signature.bin` carried by `bundle_io::{read_unsigned, write_unsigned}`. CLI extended with `seal / verify / keygen`. `tools/seal-evidence.sh` + `tools/verify-evidence.sh` shell wrappers handle key-source resolution. 13 integration tests in `tests/sign_verify.rs` cover round-trip (both labels), the 5 tamper classes (manifest / uart / trace / config / signature swap), policy rejection, cross-key rejection, unsupported version, bad magic, and unsigned-bundle verify. Placeholder `keys/evidence-ci.pub.ed25519` checked in for CI to verify against; rotation procedure documented in `keys/README.md`.
  - [x] Cut P5-04: key separation via `nexus_evidence::key::from_env_or_dir` (CI: `NEXUS_EVIDENCE_CI_PRIVATE_KEY_BASE64`; bringup: `~/.config/nexus/bringup-key/private.ed25519` with mandatory `0600` perm check; otherwise `EvidenceError::KeyMaterialMissing`). New `tools/gen-bringup-key.sh` (writes `~/.config/nexus/bringup-key/{private,public}.ed25519`) + `tools/gen-ci-key.sh` (writes `keys/evidence-ci.pub.ed25519`, prints base64 private seed for the CI secret store, never persists private key to disk). Deny-by-default secret scanner in `src/scan.rs` (PEM private-key blocks, literal `bringup-key/private` paths, `*PRIVATE_KEY*=…` env-style assignments, ≥64-char base64-looking high-entropy blobs) wired into `Bundle::seal` so a sealed bundle can never commit secret material; `scan.toml` allowlist file shipped empty. `Bundle::seal` API change: returns `Result<Bundle, EvidenceError>` (callers must handle the new `EvidenceError::SecretLeak`). `.gitignore` rejects `**/private.ed25519` and `**/*.priv.ed25519` belt-and-braces. 11 integration tests in `tests/scan.rs` (7) + `tests/key_separation.rs` (4) lock the surface.
  - [x] Cut P5-05: post-pass evidence pipeline wired into `scripts/qemu-test.sh` (single bundle at `target/evidence/<utc>-<profile>-<gitsha>.tar.gz`) and `tools/os2vm.sh` (per-node bundles `…-a.tar.gz` / `…-b.tar.gz`). Env knobs: `NEXUS_EVIDENCE_SEAL=1` (force seal), `CI=1` (implies seal + rejects `NEXUS_EVIDENCE_DISABLE=1`), `NEXUS_EVIDENCE_DISABLE=1` (skip; rejected when seal is required). Label resolution mirrors `tools/seal-evidence.sh`: CI key when `NEXUS_EVIDENCE_CI_PRIVATE_KEY_BASE64` is set, bringup key otherwise. Failure to assemble or seal is fatal (no silent skip). 5 integration tests in `tests/qemu_seal_gate.rs` lock the env-knob decision matrix.
  - [x] Cut P5-06: closure. `docs/testing/evidence-bundle.md` final pass (§3a Assembly, §3b Signing & verification, §3c Key separation, §3d Secret scanner, §5 Operational gates with the env-knob matrix and CI hard gates); RFC-0038 §"Stop conditions / acceptance" Phase 5 ticked (7 boxes, this list); `.cursor/{handoff,current_state,next_task_prep}` synced to Phase-5-closed / Phase-6-active; `tasks/STATUS-BOARD.md` + `tasks/IMPLEMENTATION-ORDER.md` refreshed; Phase-6 plan to be authored at the start of the Phase-6 session as a separate plan file.
- [ ] **Phase 6**: replay capability. Hard gates per "Phase 6 — Replay capability" above.
  - [x] Cut P6-01: `tools/replay-evidence.sh` skeleton — extract, validate, pin git-SHA, set env, invoke `just test-os PROFILE=<recorded>`. (2026-04-20; hardened with mandatory timeout + structured logs + deterministic tool resolution + persistent worktree/cache reuse)
  - [x] Cut P6-02: trace diff format spec + `tools/diff-traces.sh`; unit fixtures for "exact match", "extra", "missing", "reorder", "phase mismatch". (2026-04-20; fixtures in `docs/testing/trace-diff-fixtures.json`)
  - [x] Cut P6-03: `tools/bisect-evidence.sh` with mandatory `--max-commits` and `--max-seconds` budgets. (2026-04-20; binary-search core + fail-closed budget handling + log surface)
  - [x] Cut P6-04: `scripts/regression-bisect.sh` wrapper for the typical CI-failure flow. (2026-04-20; passes through timeout/log/worktree controls)
  - [~] Cut P6-05: cross-host determinism floor (CI runner + 1 dev box) for the same bundle; documented allowlist for non-deterministic surfaces. **Functionally done**: allowlist mechanics shipped; exact-match replay confirmed on native + containerized CI-like host for bundle `20260420T133203Z-full-b84e4c2.tar.gz` (`.cursor/replay-dev-a.json`, `.cursor/replay-ci-like.json`); synthetic bad-bundle (`.cursor/replay-synthetic-bad.json`) and 3-commit good→drift→regress bisect (`.cursor/bisect-good-drift-regress.json`) both pass with classified outputs. **Pending (environmental only)**: explicit replay artifact captured on the external project CI runner using `docs/testing/replay-and-bisect.md` §7-§8; flip to `[x]` when `.cursor/replay-ci.json` lands.
  - [x] Cut P6-06: `docs/testing/replay-and-bisect.md` documents workflow + allowlist + extension procedure. (2026-04-20)
...
- [x] Phase 4–6 add new markers only via manifest entries; all existing markers carry over byte-identical (no rename).
- [x] After Phase 4 closure, `TASK-0024` is unblocked; until then it remains gated.
