...
- **Phase 1 (structural refactor without behavior change)**: ⬜
...
- [ ] **Phase 0**: contract seed defined; initial target structure + invariants are explicit — proof: task + RFC linked
...
- [ ] **Phase 2**: broader module boundaries optimized for maintainability/extensibility — proof: same phase proof floor
...
- [ ] QEMU markers remain green in `scripts/qemu-test.sh`.
- [ ] Security-relevant negative behavior remains fail-closed.
- [ ] Any discovered logic-error or fake-success-marker path is converted into honest behavior/proof signaling.
