...
- TASK-0023B Phases 1-6 closed (capability extractions, two-axis structure, manifest split, replay/bisect tooling).
- **RFC-0061 M4**: Observer scaffolding landed (`observer/{markers, telemetry, liveness}.rs`).
  The selftest-client now has a pure-observer subsystem that reads markers from logd,
  polls display telemetry, and checks service liveness via samgrd — all without
  initiating control-plane IPC. This is the foundation for converting the 12 phases
  from active IPC callers to passive marker-readers.
- See: `docs/rfcs/RFC-0061-selftest-observer-init-refactoring.md`

> ADR scope: this ADR governs the two-axis selftest-client structure. RFC-0061's observer subsystem consumes this structure without changing its phase-dispatch contract. ADR-0027 stays `Accepted`.
