...
Files is a canonical QuerySpec consumer for data-heavy app shells.

Recommended posture:

- folder listings, provider listings, search text, tags, sort/group modes, and curated views are represented as pure query
  state,
- Files builds QuerySpec in stores/composables,
- execution happens via `contentd.query(...)` or another owning service from effects only,
- command flows such as open, rename, delete, restore, share, and grant issuance remain explicit domain actions.

This split keeps the list/grid shell deterministic while avoiding “generic query API” creep into mutation flows.

This document also defines the “Finder-like” **display + preferences contract** for the Files app.

See also:

- Folder icon theming via template + emblem overlay: `docs/dev/ui/foundations/visual/icon-guidelines.md`
- Icon sizes/strokes: `docs/dev/ui/foundations/visual/icons.md`
...
- Touch targets must remain reasonable across phone/tablet/desktop (see `docs/dev/ui/foundations/layout/profiles.md`).
...
- sort/group/filter presets may be app-specific, but should compile to the shared query contract rather than a parallel list
  implementation
...
Tag colors must be chosen from the curated palette (no arbitrary RGB by default): `docs/dev/ui/foundations/visual/colors.md`.
...
- tag/facet filtering should remain query-shaped so large tagged collections can page and virtualize predictably.
...
See the contract: `docs/dev/ui/foundations/visual/icon-guidelines.md` (“Folder icons” section).
