...
- OS/QEMU live-input contract: `docs/rfcs/RFC-0053-input-v1_0b-os-qemu-live-input-hidrawd-touchd-inputd-contract.md` (`Done`; the real `hidrawd -> inputd -> windowd -> fbdevd` live path is landed and verified, with only user-deferred repo-wide `scripts/fmt-clippy-deny.sh` / `just test-all` left outside this closeout)
