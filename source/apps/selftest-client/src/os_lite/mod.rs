...
pub fn run() -> core::result::Result<(), ()> {
    ...
    // P2-07 deviation note: `logd` was owned by `phases::policy::run` and dropped at
    // end-of-phase. The still-inline exec / TASK-0014 / journaling slices need it for
    // crash-log + hardening + metrics + journaling assertions. Re-resolve via the silent
    // `route_with_retry` (no marker change). Folded into `phases::exec::run` /
    // `phases::logd::run` at P2-08 / P2-09.
