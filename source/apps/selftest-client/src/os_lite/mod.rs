...
pub fn run() -> core::result::Result<(), ()> {
    ...
    // P2-08 deviation note: `logd` was owned by `phases::exec::run` (crash-log
    // verification) and dropped at end-of-phase. The still-inline TASK-0014 /
    // metrics / journaling slices need it for hardening + metrics + journaling
    // assertions. Re-resolve via the silent `route_with_retry` (no marker change).
    // Folded into `phases::logd::run` at P2-09.
