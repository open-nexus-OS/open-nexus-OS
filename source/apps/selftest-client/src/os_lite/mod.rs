...
pub fn run() -> core::result::Result<(), ()> {
    ...
    // P2-06 deviation note: the still-inline policy slice (collapsed at P2-07) needs
    // a fresh `bundlemgrd` for the `bundlemgrd_v1_route_status("execd")` deny proof.
    // `route_with_retry` is silent (no markers), so this preserves the marker ladder.
