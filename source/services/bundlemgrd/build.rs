...
fn main() {
    ...
    // GET_PAYLOAD source (TASK-0080D): compile each ui-program bundle's DSL
    // app to its canonical `.nxir` and embed it — bundlemgrd SERVES payloads,
    // it never interprets them. Source-root convention until the TASK-0081
    // consolidation moves apps into `userspace/apps/<name>/ui/`:
    // `examples/dsl/<name>/<name>.nx`. A ui-program bundle whose source is
    // missing is a BUILD failure (fail-closed, no phantom payloads).
    let _ = writeln!(
        out,
    ...
    for id in &ui_programs {
        let src = manifest_dir.join(format!("../../../examples/dsl/{id}/{id}.nx"));
        println!("cargo:rerun-if-changed={}", src.display());
        let nxir = compile_dsl_payload(&src)
...
/// Compiles a DSL source file to its canonical `.nxir` bytes (the same
/// parse→check→format→lower chain as `nx dsl build`; byte-deterministic, so
/// the served payload hash-matches every other compile of the same source).
fn compile_dsl_payload(src: &Path) -> Result<Vec<u8>, String> {
    let source = std::fs::read_to_string(src)
        .map_err(|e| format!("read {}: {e}", src.display()))?;
    let file = nexus_dsl_core::parse_file(&source)
        .map_err(|d| format!("parse: {} {}", d.code, d.message))?;
    let (model, diags) = nexus_dsl_core::check_file(&file);
    if nexus_dsl_core::has_errors(&diags) {
        return Err(format!("check: {diags:?}"));
    }
    let canonical = nexus_dsl_core::format_file(&file);
    let lowered = nexus_dsl_core::lower_file(&file, &model, &canonical)
        .map_err(|d| format!("lower: {} {}", d.code, d.message))?;
    Ok(lowered.nxir)
}
