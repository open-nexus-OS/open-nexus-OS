...
fn main() {
    ...
    // (id, display label, bundle_type). The registry knows ALL installed
    // bundles; the enumerate/list view filters by type (only `app` is
    ...
    let _ =
        writeln!(out, "pub(crate) const APP_PAYLOADS: [(&str, &[u8]); {}] = [", ui_programs.len());
...
fn read_app(manifest: &Path) -> Option<(String, String, String, String)> {
    ...
    // Bundle role/privilege class (manifest.capnp BundleType). Absent = "app".
