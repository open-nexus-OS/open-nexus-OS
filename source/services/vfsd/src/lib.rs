...
fn handle_mount(dispatcher: &Dispatcher, payload: &[u8]) -> Result<Vec<u8>> {
    ...
    match dispatcher.mount(&mount_point, &fs_id) {
        ...
        Err(ServerError::Service(ServiceError::Provider(_))) => encode_mount_response(false),
        Err(err) => Err(err),
    }
}

fn encode_open_response(ok: bool, fh: u32, size: u64, kind: u16) -> Result<Vec<u8>> {
    let mut message = capnp::message::Builder::new_default();
    {
        let mut response = message.init_root::<open_response::Builder<'_>>();
        response.set_ok(ok);
        response.set_fh(fh);
        response.set_size(size);
        response.set_kind(kind);
    }
    serialize_response(OPCODE_OPEN, message)
}

fn encode_read_response(ok: bool, bytes: &[u8]) -> Result<Vec<u8>> {
    let mut message = capnp::message::Builder::new_default();
    {
        let mut response = message.init_root::<read_response::Builder<'_>>();
        response.set_ok(ok);
        if ok {
    ...
    serialize_response(OPCODE_READ, message)
}

fn encode_close_response(ok: bool) -> Result<Vec<u8>> {
    let mut message = capnp::message::Builder::new_default();
    {
        let mut response = message.init_root::<close_response::Builder<'_>>();
        response.set_ok(ok);
    }
    serialize_response(OPCODE_CLOSE, message)
}

fn encode_stat_response(ok: bool, size: u64, kind: u16) -> Result<Vec<u8>> {
    let mut message = capnp::message::Builder::new_default();
    {
        let mut response = message.init_root::<stat_response::Builder<'_>>();
        response.set_ok(ok);
        response.set_size(size);
        response.set_kind(kind);
    }
    serialize_response(OPCODE_STAT, message)
}

fn encode_mount_response(ok: bool) -> Result<Vec<u8>> {
    let mut message = capnp::message::Builder::new_default();
    {
        let mut response = message.init_root::<mount_response::Builder<'_>>();
        response.set_ok(ok);
    }
    serialize_response(OPCODE_MOUNT, message)
}
...
/// Ensures Cap'n Proto schemas are linked.
pub fn touch_schemas() {
    let _ = core::any::type_name::<open_request::Reader<'static>>();
    let _ = core::any::type_name::<read_request::Reader<'static>>();
    let _ = core::any::type_name::<close_request::Reader<'static>>();
    let _ = core::any::type_name::<stat_request::Reader<'static>>();
    let _ = core::any::type_name::<mount_request::Reader<'static>>();
}
