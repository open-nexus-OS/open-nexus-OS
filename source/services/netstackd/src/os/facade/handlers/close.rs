...
//! CONTEXT: OP_CLOSE handler for netstackd IPC facade
//! OWNERS: @runtime
//! STATUS: Experimental
...
use crate::os::ipc::wire::{OP_CLOSE, STATUS_MALFORMED, STATUS_NOT_FOUND, STATUS_OK};
...
pub(crate) fn handle<R: FnMut(&[u8])>(
    ...
    if validation::validate_exact_or_nonce_len(req.len(), 8).is_malformed() {
        reply(&status_frame(OP_CLOSE, STATUS_MALFORMED));
        let _ = yield_();
    ...
    let Some(sid) = StreamId::from_wire(sid) else {
        reply_status_maybe_nonce(reply, OP_CLOSE, STATUS_NOT_FOUND, nonce);
        let _ = yield_();
        return DispatchControl::ContinueLoop;
    };
    let sid0 = sid.index();
