...
use super::{
    DARK_GLASS_RADIUS, SHADOW_BOX_CACHE_ENTRIES, SHADOW_CACHE_MAX_DOWNSCALE,
    SOFT_PANEL_SHADOW_ALPHA, SOFT_PANEL_SHADOW_BLUR_RADIUS, SOFT_PANEL_SHADOW_OFFSET_Y,
...
pub(crate) fn composite_shadow_layer_row(
    target: &mut [u8],
    layer_row: &[u8],
    cached_width: u32,
    logical_width: u32,
...
pub(crate) fn draw_soft_panel_shadow_row(y: u32, target: &mut [u8], rect: ProofBoxRect) {
    ...
    let ex = (rect.x as i32)
        .saturating_add(rect.width as i32)
        .saturating_add(blur)
        .max(0)
        .min(rp as i32) as usize;
    if (y as i32) < sy.saturating_sub(blur)
        || (y as i32) > sy.saturating_add(rect.height as i32).saturating_add(blur)
    {
        return;
    }
    ...
    for px in sx..ex {
        ...
        let a = fixed_sdf::shadow_alpha_from_distance(
            sd.max(0),
...
pub(crate) fn compute_shadow_row(
    _st: VisibleState,
    pl: Option<&LayoutResult>,
    pli: Option<&LayoutHotPathIndex>,
    y: u32,
    tgt: &mut [u8],
    ss: &mut [u8],
    bb: &mut [u8],
    arena: &mut ShadowArena<'_>,
    _cs: &mut [u8],
    sbc: &mut [ShadowBoxCacheEntry; SHADOW_BOX_CACHE_ENTRIES],
) -> Result<(), WindowdError> {
    let Some(ly) = pl else {
        return Ok(());
    };
    ...
    let mut ds = |lb: &nexus_layout::LayoutBox| {
        ...
        let cw = (sw + 2 * bi).max(0) as u32;
        let ch = (sh2 + 2 * bi).max(0) as u32;
        let bid = lb
            .id
            .map(|s| {
                let mut h: u64 = 0;
        ...
        if let Some(sl) = cs {
            ...
            let ly2 = y.saturating_sub(re.y as u32);
            let cr2 = ly2 as usize / e.scale as usize;
            let rs = cr2 * e.cache_width as usize * 4;
            let re2 = rs + e.cache_width as usize * 4;
            if re2 <= lr.len() {
        ...
        let sc = shadow_cache_scale(cw, ch, WINDOWD_SHADOW_ARENA_SIZE)
            .unwrap_or(SHADOW_CACHE_MAX_DOWNSCALE);
        let acw = cw.div_ceil(sc as u32).max(1);
        let ach = ch.div_ceil(sc as u32).max(1);
        let ab = acw as usize * ach as usize * 4;
        if let Some((off, buf)) = arena.alloc(ab) {
            for cy in 0..ach {
                draw_shadow_row_fallback(
                    (cy * sc as u32) as u32,
                    &mut buf[cy as usize * acw as usize * 4..][..acw as usize * 4],
                    ss,
                    bb,
                    (acw * 4 / 4) as usize,
                    sx / sc as i32,
            ...
            let ly2 = y.saturating_sub(re.y as u32);
            let cr2 = ly2 as usize / sc as usize;
            composite_shadow_layer_row(
                tgt,
                &buf[cr2 * acw as usize * 4..][..acw as usize * 4],
                acw,
                cw,
                sc,
                sx,
            ...
            draw_shadow_row_fallback(y, tgt, ss, bb, rp, sx, sy2, sw, sh2, bi, br, sh.color);
        }
    };
    match rm {
        Some(mut m) => {
...
pub(crate) fn draw_shadow_row_fallback(
    y: u32,
    tgt: &mut [u8],
    ss: &mut [u8],
    bb: &mut [u8],
    ...
    let yi = y as i32;
    let dy = if yi < sy2 {
        sy2.saturating_sub(yi)
    } else if yi >= sy2.saturating_add(sh2) {
        yi.saturating_sub(sy2.saturating_add(sh2).saturating_sub(1))
    } else {
        0
    };
    if dy > bi {
    ...
    let va = if br == 0 {
        ...
        let r = bi.saturating_add(1).saturating_sub(dy) as u32;
        (r * 255) / (br + 1)
    };
    if va == 0 {
        return;
    ...
    let eb = en * 4;
    ss[sb..eb].fill(0);
    let cs = sx.max(0).min(rp as i32) as usize;
    let ce = sx.saturating_add(sw).max(0).min(rp as i32) as usize;
    for px in cs..ce {
        ss[px * 4 + 3] = va as u8;
    }
    let sl = eb.saturating_sub(sb);
    if br > 0 && sl != 0 {
        super::blur_row_horizontal(&mut ss[sb..eb], sl, br, bb);
    }
    let sr = col.r as u32;
    let sg = col.g as u32;
    let sb2 = col.b as u32;
    let sa = col.a as u32;
    for px in st..en {
        let idx = px * 4;
        let la = ss[idx + 3] as u32;
        if la == 0 {
            continue;
        }
        let ta = (la * sa) / 255;
