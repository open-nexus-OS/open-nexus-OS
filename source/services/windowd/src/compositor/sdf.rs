...
pub(crate) fn stroke_dark_glass_border_row(
    y: u32,
    row: &mut [u8],
    rect: ProofBoxRect,
    rc: super::types::RenderClip,
    stroke: u32,
    bgra: [u8; 4],
) -> Result<(), WindowdError> {
    if stroke == 0 {
    ...
    let mny = fixed_sdf::px_u32(rect.y);
    ...
    for px in st..en {
        let sd =
            fixed_sdf::rounded_rect_sd(fixed_sdf::pixel_center(px), py, mnx, mny, mxx, mxy, rad);
...
pub(crate) fn fill_sdf_circle_row(
    y: u32,
    row: &mut [u8],
    x: u32,
    ry: u32,
    ...
    let cy = ry as f32 + h as f32 * 0.5;
    let r = w.min(h) as f32 * 0.5;
    for px in x.max(0)..(x + w).min(rp) {
        let a = nexus_sdf::fill_alpha(
            nexus_sdf::sd_circle((px as f32 + 0.5, y as f32 + 0.5), (cx, cy), r),
            1.0,
        );
        if a > 0.0 {
            let idx = px as usize * 4;
            let fa = (a * bgra[3] as f32) as u32;
            if fa == 0 {
                continue;
            }
            let inv = 255 - fa;
...
pub(crate) fn stroke_sdf_circle_row(
    y: u32,
    row: &mut [u8],
    x: u32,
    ry: u32,
...
// (RFC-0067 P5-Final G2: `fill_sdf_rounded_rect_row` + `stroke_sdf_rounded_rect_row`
// — the CPU rounded-rect glass fill/stroke — removed; dead on both backends
// (G0 `cpu-sdf-fill` never fired). Rounded glass containers are GPU-rendered;
// the CPU content path draws square content boxes only.)
