...
#[test]
fn focus_gates_delivery_end_to_end() {
    let mut core = ImedCore::new();
    // Unfocused: composed but NOT delivered (RFC-0075 Phase 3 — the osk
    // probe echo still observes the step; delivery stays focus-gated).
    assert_eq!(core.key(wire::KEY_KIND_TEXT, u32::from('a'), 0).0, None);
    core.set_focus(3, true, wire::FIELD_KIND_TEXT);
    let push = core.key(wire::KEY_KIND_TEXT, u32::from('a'), 0).0.expect("push");
    assert_eq!(push.surface_id, 3);
    assert_eq!(push.commit.expect("commit").as_str(), "a");
    // Unfocus gates delivery again.
    core.set_focus(3, false, wire::FIELD_KIND_TEXT);
    assert_eq!(core.key(wire::KEY_KIND_TEXT, u32::from('b'), 0).0, None);
}
...
fn de_dead_key_composition_round_trip() {
    ...
    assert_eq!(core.key(wire::KEY_KIND_DEAD, u32::from('´'), 0).0, None);
    let push = core.key(wire::KEY_KIND_TEXT, u32::from('e'), 0).0.expect("push");
...
fn test_reject_invalid_key_payloads() {
    let mut core = ImedCore::new();
    core.set_focus(1, true, wire::FIELD_KIND_TEXT);
    assert_eq!(core.key(0xFF, u32::from('a'), 0).0, None, "unknown kind");
    assert_eq!(core.key(wire::KEY_KIND_TEXT, 0xD800, 0).0, None, "surrogate scalar");
    assert_eq!(core.key(wire::KEY_KIND_ACTION, 0, 0xFF).0, None, "unknown action");
}
