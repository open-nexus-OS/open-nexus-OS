...
impl VirtioGpuBackend {
    ...
    pub(crate) fn release_resource(&mut self, res: ResourceId) {
        ...
        // RFC-0085: REALLY unmap the backing (the doc used to claim the slot
        // became reusable while the pages stayed mapped forever). Must happen
        // BEFORE `vmo_destroy` — the kernel now refuses EBUSY while any
        // region still maps the range.
        if record.backing_map_len != 0 {
            if let Err(err) = nexus_abi::vm_unmap(record.backing_va, record.backing_map_len) {
                crate::diag::err_line(b"release unmap", err);
            }
        }
