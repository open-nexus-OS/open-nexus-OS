...
impl VirtioGpuBackend {
    ...
    pub fn probe(&mut self) -> Result<(), GpuDriverError> {
        ...
        // ADR-0050). Authority = the fw_cfg-configured mode (kernel-derived,
        // race-free); the device's GET_DISPLAY_INFO reply is validated CAPABILITY
        // data, never authority — under QEMU's GTK backend it transiently reports
        // the un-realized window's default size, which used to latch a wrong mode.
        // The RESOURCE layout stays at the fixed maximum; this sizes the visible
        // sub-rect. gpud then commands its bootstrap scanout at this resolved mode.
        #[cfg(all(feature = "os-lite", target_os = "none"))]
        {
            let device = if self.ctrlq.is_some() {
                self.ctrl_query_display_info()
            } else {
                None
            };
            if let Some((w, h)) = device {
                // Diagnostic only: what the device advertised (capability), not
                // what we command — the two can differ under the GTK race.
                emit_display_info_marker(w, h);
            }
            let configured = nexus_abi::boot_display_mode();
            let (rw, rh) = resolve_display_mode(configured, device, (1280, 800));
            self.display_w = rw;
            self.display_h = rh;
            if configured.is_none() && device.is_none() {
                let _ = nexus_abi::trace_line("gpud: display mode fallback max (1280x800)");
            }
        }
